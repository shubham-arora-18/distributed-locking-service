...
def test_invalid_addition_of_already_added_write_process(add_write_process):
...
def test_valid_add_and_get_timed_out_lock(mock_process, add_write_process):
    ...
    assert len(response.json()["write_process_list"]) == 1
    assert not any(
        rp.get("process_id") == rand_process_id for rp in response.json().get("write_process_list")
    )


@mock.patch("distributed_locking_service.services.distributed_lock.Process")
def test_valid_add_and_get_lock(mock_process, add_write_process):
    rand_process_id = get_random_process_id()
    past_time = datetime.now(timezone.utc) - timedelta(seconds=10)
...
def test_exclusive_lock_with_two_put_calls(created_exclusive_write_distributed_lock):
    ...
    response_2 = client.put(
        f"/v1/distributed_lock/{rand_lock_id}/read-process/{rand_process_id_2}",
        headers=valid_headers,
        params=parameters,
    )
    assert response.status_code == 200
    assert "READ" == response_2.json()["current_state"]
    assert len(response_2.json()["read_process_list"]) == 1
    assert any(
        rp.get("process_id") == rand_process_id_2
        for rp in response_2.json().get("read_process_list")
    )
